...
- **Две ветки в одной функции** `chatResolve(text)`. Без ключа — локальный разбор
  (`chatResolveLocal`: `CHAT_RULES` для простых команд + `asstParse` для поиска);
  с ключом — модель через `askModel`. Форма ответа одна: `{ calls, reply?, note?, executed? }`
  (`calls` выполняет чат, `executed` уже выполнены внутри цикла модели). Лента,
  рендер, клики и undo от ветки не зависят.
- **`js/ai.js`** — вызов модели, BYOK. Провайдер сменный (реестр `AI_PROVIDERS`: `url`,
  `buildBody`, `parseReply`, `listModels`, форматы ходов); реализован Google Gemini
  (`generateContent`, CORS из браузера работает — бэкенд не нужен).
  **Список моделей — живой, из API** (`listModels()` → `GET /v1beta/models`, фильтр по
  `generateContent`, кэш на сессию): имена моделей у Google закрываются без
  предупреждения (`gemini-2.5-flash-lite` перестал выдаваться новым ключам), поэтому
  хардкод списка — источник поломок. Константа `models` в записи провайдера — только
  запасной вариант до первого ответа API. Селектор в настройках подтягивает список
  при вводе ключа и чинит сохранённую модель, если её закрыли. `askModel` крутит агентный цикл (≤3 оборота): вызов → исполнение
  команд → результаты обратно модели → текстовый ответ. Модели уходит **сводка**
  результата (`found`, первые JD), а не сотни строк — полный список рисует чат.
  Схемы инструментов конвертируются под OpenAPI-подмножество Gemini (`toGeminiSchema`:
  вырезает `additionalProperties`, опускает пустые `properties`/`required`).
- **Ключ — в localStorage этого браузера и только**. Намеренно НЕ запись реестра
  настроек: значения реестра синхронизируются в Supabase `user_metadata`, личному ключу
  там не место. Запись `aiKey` — `type:'custom'`, чистый UI (раздел «Ассистент»,
  `section: 'ai'`), чтение/запись через `getAiKey`/`setAiKey` из ai.js.
- **Что модель видит** — `chatContext()`: имя и тип кейса, натальные позиции (уже
  посчитанные эфемеридами), As/Mc, текущий режим и момент, активный виджет. Системный
  промпт `CHAT_SYSTEM` запрещает считать что-либо самостоятельно и называть даты,
  которых не было в результате инструмента.
