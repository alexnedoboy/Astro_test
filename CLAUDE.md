...
### Библиотека (`#v2-library-view`)
- Вторая **выдвижная панель** в том же месте, что и заметки (`position:absolute` внутри
  `#v2-aspect-panel`, класс `library-open`); открыть — иконка `#libraryToggleBtn` в хедере
  рядом с заметками. Панели взаимно исключают друг друга, обе закрываются при выходе на
  дашборд. В отличие от заметок, библиотека **не привязана к кейсу** — это глобальный
  справочник, при смене вкладки не перезагружается.
