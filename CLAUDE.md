...
- **`js/ai.js`** — вызов модели, BYOK. Провайдер сменный (реестр `AI_PROVIDERS`: `url`,
  `buildBody`, `parseReply`, форматы ходов); реализован Google Gemini
  (`generateContent`, free tier `gemini-2.5-flash-lite`, CORS из браузера работает —
  бэкенд не нужен). `askModel` крутит агентный цикл (≤3 оборота): вызов → исполнение
