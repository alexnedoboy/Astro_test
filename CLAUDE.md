...
Весь код — в одном файле: **`index.html`** (~5350 строк).
...
- Блок под картой (`#horary-block`, `renderHoraryBlock`): градусная линейка 0–30° (`buildDegreeRuler`) + транспонированная таблица достоинств (`buildHoraryDigTable`).
- Панель режимов и таймлайн выключены; правая группа предзагружает все виджеты, активен «Аспекты ±».

### Правая группа виджетов (`#v2-aspect-panel`)
Группа переключаемых виджетов-табов (см. WORKSPACE_CONCEPT.md §6, §13):
- Реестр `RP_WIDGETS`: `grades` (таблица планет) / `aspects` (сетка) / `dignities` (эссенциальные
  достоинства, `renderDignities`) / `aspflow` («Аспекты ±»: сепарации/аппликации Луны в знаке +
  пары в орбе, `renderAspFlow`) / `notes` (заметки). Новые виджеты добавляются сюда.
