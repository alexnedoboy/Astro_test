...
### Лаборатория: навигация под картой (`labChartNav`)
Пробный вид, включается тумблером в разделе «Лаборатория» модалки настроек
(`section: 'lab'`, ls-ключ `labChartNav`). Одна точка применения —
`applyLabChartNav` → класс `lab-chartnav` на `body`; выключается на ходу, без
перезагрузки. Что меняется:
- на открытом кейсе (`body.home-off`) прячется `#app-header` и обнуляется
  `--hdr-h` (базовое значение вынесено в `--hdr-h-base`) — карта забирает высоту;
  на дашборде хедер остаётся: это точка входа;
- контроллер времени в колонне (`#v2-panel-nav`) скрыт, вместо него строка
  `#v2-chart-nav` под картой: `◀◀◀ ◀◀ ◀ [дата время] ▶ ▶▶ ▶▶▶`, справа ↺ и
  сегменты Дата/Время (класс `.v2-navmode-btn` — общий с колонной, оба
  переключателя синхронны). Раскладка — грид пяти областей
  `lock prev date next right`; на ≤950px те же области перекладываются в два
  ряда (дата сверху, крупные стрелки под ней по краям) — под палец на планшете;
- панель карты сужена до `aspect-ratio: 15/16` (`#v2-chart-area`, кроме
  `chart-expanded`): круг рисуется по меньшей стороне, поэтому лишние поля по
  бокам (до ~90px) уходят правой панели — она единственная растёт;
- шаг обоих контроллеров — одна функция `runNavAction(nav)`; подпись рисует
  `syncV2DateLabel()` сразу в оба места, подсветка сдвинутого момента —
  `.cn-shifted` / `.v2-date-active`;
- в натальном режиме стрелки заблокированы (`cn-locked`, замок `#cn-lock`,
  `syncChartNavLock`) — случайный клик не сдвинет момент рождения; ↺ работает
  всегда, замок возвращается при уходе из натала; элекция не блокируется;
- хедера нет → библиотека/заметки/настройки/выход живут в `#v2-bottom-row`
  колонны данных (кнопки `.lab-only` показываются только в этом виде);
- на ≤700px эксперимент выключен (строка не влезает в один ряд) — мобильная
  раскладка остаётся штатной.

### Палитра (`#palette-view`)
Кнопка `+` таб-бара открывает пустую вкладку с палитрой (создать/открыть кейс, WORKSPACE_CONCEPT §8).
**Виды вкладок — реестр `TAB_KINDS`** (`kind`: `'palette' | 'library' | undefined`; кейс — вкладка
без `kind`, `type` — это ортогональный тип кейса). Запись реестра: `label` / `show` / `hide` плюс
необязательные `snapshot(t)` / `serialize(t)` / `hydrate(d)` — они нужны вкладке со своим состоянием
(библиотека помнит место чтения), палитре нечего помнить. Проверки «это не кейс» — просто `if (t.kind)`:
не снимать снапшот кейса, не писать в hash, не сохранять в базу. Выбор карты или расчёт формы
превращает палитру во вкладку кейса в том же слоте (`choosePaletteChart` / `computeIntoPaletteTab`). Список карт — из `_homeCharts` (pinned → `last_opened_at`).
...
### Библиотека (`createLibraryUI`)
- Читалка — **фабрика** `createLibraryUI(root, {onClose, onPopOut, wide})`: разметку строит
  сама, поэтому хосту нужен пустой контейнер. Экземпляров два, у каждого своё положение
  в книге; `libIndex` и `libChapterCache` общие на сессию.
- **Хост «панель»** (`#v2-library-view`, глобал `libPanel`) — выдвижная панель в том же
  месте, что и заметки (`position:absolute` внутри `#v2-aspect-panel`, класс `library-open`);
  открыть — иконка `#libraryToggleBtn` в хедере. Панели взаимно исключают друг друга, обе
  закрываются при выходе на дашборд. Библиотека **не привязана к кейсу** — при смене
  вкладки не перезагружается. Кнопка ⤢ (`libraryPanelToTab`) раскрывает панель во вкладку
  с того же места.
- **Хост «вкладка»** (`#library-tab-view`, глобал `libTab`, `wide: true`) — полотно на всё
  окно: оглавление колонкой слева (не сворачивается), текст колонкой ~68ch. Создаётся
  командой «Библиотека» в палитре (`becomeLibraryTab` — палитра становится библиотекой
  в том же слоте) или кнопкой ⤢ из панели. Вкладка помнит книгу, главу и позицию
  прокрутки (`TAB_KINDS.library.snapshot/serialize/hydrate` + `state()`/`restore()`
  фабрики) — при переключении вкладок и после перезагрузки чтение продолжается с того
  же места. В `restore()` есть страховка: если индекс не загрузился (нет логина),
  положение сохраняется, но не рисуется — иначе сбрасывалось бы на полку.
- **Содержимое** — учебники Авессалома Подводного (4 книги, 61 глава, ~5 МБ markdown) в
  **приватном** бакете Supabase Storage `library`: `index.json` (дерево книга → глава →
  раздел, ~120 КБ) и `<book>/<chapter>.md`. Тексты под авторским правом: бакет читает
  только `authenticated` (`supabase_library.sql`), в git не коммитятся (`.gitignore`).
- **Сборка и заливка** — вне приложения, из Obsidian-хранилища `../Astrology`:
  `node tools/build-library.mjs` → `library-dist/`, затем
  `SUPABASE_SERVICE_KEY=… node tools/upload-library.mjs`. Индекс полностью производный от
  заголовков (`#` — глава, `##`/`###` — разделы) и фронтматтера; в сами .md ничего дописывать
  не нужно.
- **Экраны** одной панели (`libView.screen`): `shelf` (книги) → `book` (главы, служебные —
  в хвосте по флагу `aux`) → `chapter` (текст) + `hits` (несколько подходящих разделов).
  Поиск `#lib-search` — по заголовкам глав и разделов из индекса (в экране книги сужен до неё).
- **Читалка**: markdown рендерит общий `renderMarkdown`; якоря заголовкам присваиваются по
  порядку из `chapter.sections` (слаг считается только в сборщике, в рантайме не дублируется).
  Мини-оглавление `#lib-reader-toc` липкое: развёрнуто у верха, ниже сворачивается в строку с
  текущим разделом. Прокрутка к разделу — `libScrollToHead` (+ `libHoldAnchor`: пару секунд
  удерживает якорь, пока плывут высоты длинной главы; отменяется первым ручным скроллом).
- **Контекстные ссылки**: строки таблицы планет и домов (`buildInfoTable`) несут
  `data-lib-subject`/`data-lib-sign`; клик ищет в индексе разделы с заголовком
  «‹субъект› в|во ‹знак›» (`libFindSections`) — «Луна в Козероге», «Пятый дом в Стрельце».
  Один результат открывается сразу, несколько (одна тема в разных книгах) — списком.
- Кэш: `libIndex` и `libChapterCache` живут на сессию, в localStorage ничего не пишется.
